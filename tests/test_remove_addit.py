...
def test_no_additions_keeps_same_content_and_object_identity():
    al = AL("ABC", [10, 20, 30])
    al_id = id(al)
    r = remove_additions(al, [])
    # Function mutates in place and returns nothing when no additions are provided.
    assert r is None
    assert id(al) == al_id
    assert chars_of(al) == ["A", "B", "C"]
    assert durs_of(al) == [10, 20, 30]
...
def test_total_duration_preserved(chars, durs, additions):
    ...
    remove_additions(al, additions)
    total_after = sum(durs_of(al))
...
def test_even_sized_chunk_in_middle_splits_left_half_to_left_and_right_half_to_right():
    ...
    remove_additions(al, [1, 2])
    assert chars_of(al) == ["A", "D"]
    assert durs_of(al) == [300, 700]
...
def test_odd_sized_chunk_in_middle_splits_with_middle_split_between_neighbors():
    ...
    remove_additions(al, [1, 2, 3])
    assert chars_of(al) == ["A", "E"]
    assert durs_of(al) == [450, 1050]  # A:100+200+150; E:500+400+150


def test_chunk_at_start_pushes_all_to_next():
    # Remove [0,1] → 10+20 added to C(30)
    al = AL("ABCD", [10, 20, 30, 40])
    remove_additions(al, [0, 1])
    assert chars_of(al) == ["C", "D"]
    assert durs_of(al) == [60, 40]


def test_chunk_at_end_pushes_all_to_previous():
    # Remove [2,3] → 30+40 added to B(20)
    al = AL("ABCD", [10, 20, 30, 40])
    remove_additions(al, [2, 3])
    assert chars_of(al) == ["A", "B"]
    assert durs_of(al) == [10, 90]
...
def test_multiple_disjoint_chunks_start_and_end_handled_separately():
    ...
    # Additions: [0,1] (start chunk) and [4,5] (even internal chunk)
    # - Start chunk [0,1] → 10+20 added to C (30→60)
    # - Internal even chunk [4,5]: split left/right → D gets +50, G gets +60
    al = AL("ABCDEFG", [10, 20, 30, 40, 50, 60, 70])
    remove_additions(al, [0, 1, 4, 5])
    # Remaining chars: C, D, G
    assert chars_of(al) == ["C", "D", "G"]
    # C: 30+(10+20)=60; D: 40+50=90; G: 70+60=130
    assert durs_of(al) == [60, 90, 130]
    # Total preserved
    assert sum(durs_of(al)) == sum([10, 20, 30, 40, 50, 60, 70])
...
def test_multiple_disjoint_chunks_with_odd_internal_chunk_and_boundaries():
    ...
    # - Start [0]: 100 → to B
    # - Middle [2,3,4]: left [2]=300, right [4]=500, middle [3]=400 → 200/200 split
    #   B gets +300 + 200; F gets +500 + 200
    al = AL("ABCDEF", [100, 200, 300, 400, 500, 600])
    remove_additions(al, [0, 2, 3, 4])
    assert chars_of(al) == ["B", "F"]
    assert durs_of(al) == [200 + 100 + 300 + 200, 600 + 500 + 200]  # [800, 1300]


# --- Rounding edge cases ------------------------------------------------------
# (Add rounding-specific tests here if the implementation rounds durations.)
...
def test_non_uniform_durations_collapse_to_single_char():
    ...
    remove_additions(al, [0, 1, 2])
    assert al == [CharAlignment(character="C", duration=400)]
...
def test_out_of_range_index_raises_index_error():
    ...
    with pytest.raises(IndexError):
        remove_additions(al, [3])  # 3 is out of range for len=3
...
def test_largeish_case_multiple_chunks_mixed():
    ...
    remove_additions(al, [0, 2, 3, 7, 8, 9])
    ...
    #
    # Remaining indices: 1(B), 4(E), 5(F), 6(G)
    assert chars_of(al) == ["B", "E", "F", "G"]
    assert durs_of(al) == [30, 45, 30, 170]
    assert sum(durs_of(al)) == sum([5,10,15,20,25,30,35,40,45,50])
